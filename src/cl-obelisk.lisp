...
;; --- 1. Strutture Dati ---
(defclass nodo-mappa ()
  ((id       :initarg :id       :accessor nodo-id)
   (livello  :initarg :livello  :initform 0   :accessor nodo-livello)
   (centro-p :initarg :centro-p :initform nil :accessor centro-p)
   (figli    :initform nil      :accessor nodo-figli)))
...
(defun calcola-attributi-dimensione (nodo)
  (let* ((lvl (nodo-livello nodo))
         ;; Font: calo drastico (24, 16, 11, 8...)
         (f-size (case lvl
                   (0 26)   ; Centro dominante
                   (1 18)   ; Rami principali
                   (2 12)   ; Sotto-argomenti
                   (t 9)))  ; Dettagli minimi
         ;; Spessore bordo: (4.0, 2.0, 1.0, 0.5)
         (p-width (case lvl
                    (0 4.5)
                    (1 2.5)
                    (2 1.2)
...
;; --- 3. Protocollo cl-dot (Corretto per stili e shape) ---
...
;; --- 4. Parser DSL e Preparazione ---
(defun parse-smart-dsl (parent node-data &optional (seen (make-hash-table :test 'equal)))
  "Trasforma un albero DSL in relazioni piatte, gestendo cicli e referenze."
...
;; --- 5. Funzioni Pubbliche di Esportazione ---
  ...
  (let* ((root-id (or centro-id (first (first relazioni))))
    ...
    (multiple-value-bind (nodi edge-styles) (prepara-mappa relazioni root-id)
      (let* ((graph-attrs `(:rankdir ,rankdir :overlap "false" :splines "true"
        ...
        (format t "~%[cl-obelisk] ~A generato in: ~A~%" (string-upcase ext) out-file)))))

(defun genera-da-dsl (nome-file dsl-data &rest args)
  "Genera una mappa partendo dalla struttura DSL annidata."
